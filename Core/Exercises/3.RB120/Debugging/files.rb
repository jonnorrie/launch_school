...
class File
  ...
  def to_s
    "#{name}.#{FORMAT}" # This referenace to the FORMAT Constant doesnt know there are constants with this 
  end # name in each of it's sub classes, so we need to reference the sub class like so self.class::FORMAT
end
